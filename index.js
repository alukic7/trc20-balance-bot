...
bot
  .setMyCommands([
    ...
    {
      command: '/balance',
      description: 'Get total balance of all stored wallets',
    },
  ])
  .then(() => {
...
bot.onText(/\/add/, msg => {
  ...
  bot.sendMessage(
    msg.chat.id,
    'Send TRC20 wallet addresses one by one. Type /done when finished.'
  )
})
...
bot.onText(/\/done/, msg => {
  ...
  if (!addingMode) {
    bot.sendMessage(
      msg.chat.id,
      '⚠️ You are not in address-adding mode. Use /add to start.'
    )
    return
  }
  addingMode = false
...
bot.on('message', msg => {
  ...
  if (parts.length < 2) {
    bot.sendMessage(
      chatId,
      '⚠️ Please provide both the wallet name and the TRC20 wallet address in the format: <walletName> <address>'
    )
    return
  }
  ...
  if (!address.startsWith('T')) {
    bot.sendMessage(
      chatId,
      '⚠️ Invalid TRC20 wallet address. Please try again.'
    )
    return
  }
...
bot.onText(/\/list/, msg => {
  ...
  const addresses = Array.from(
    wallets,
    ([walletName, address]) => `${walletName}: ${address}`
  )

  if (addresses.length === 0) {
    bot.sendMessage(
      chatId,
      '🚫 No wallet addresses stored. Use /add to add some.'
    )
  } else {
    bot.sendMessage(
      chatId,
...
bot.onText(/\/balance/, async msg => {
  ...
  try {
    for (let i = 0; i < walletsArray.length; i++) {
      ...
      message += `${numberToEmoji(
        i + 1
      )} ${walletName} (${address}): ${currBalance.toFixed(0)}\n\n`
    }
    bot.sendMessage(
      chatId,
